...
export const ReviewsList = () => {
	return (
		<div className='pt-5 flex items-start h-max justify-center gap-5'>
			{reviews.map((review, index) => (
				<article
					key={index}
					className='flex flex-col  gap-5 w-full max-w-[430px] h-full min-h-[680px] p-10 rounded-30 bg-muted flex-grow'
				>
					...
					<div className='flex flex-col gap-5'>
						...
						<p className='text-2xl text-muted-foreground'>{review.niche}</p>
					</div>
				</article>
			))}
		</div>
